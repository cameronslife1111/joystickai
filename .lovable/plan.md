...
- Before each sentence, Orby currently requests `navigator.audioSession.type = "ambient"`, then falls back to `auto`. Ambient can mix but does not request ducking; `auto` can let WebKit choose an exclusive mode that stops other audio.
...
1. Go to the iPhone audio-session helper and add a dedicated sentence-speech request that tries `transient` first for best-effort ducking, falls back to `ambient` for mixing, and never sets `auto`, `playback`, or `transient-solo` for sentence speech.
...
3. Go to the utterance finish, error, and cancel paths and restore `ambient` after Orby stops talking, without creating a silent audio element or claiming exclusive playback; if iOS rejects `ambient`, leave the current session untouched rather than selecting `auto`.
...
6. Go to the in-app microphone boundary and keep `play-and-record` only while Orby is recording; when recording ends, retry restoring `ambient` during iOS’s delayed hand-back. Starting sentence speech may finish Orby’s own held microphone, but it will not access or deliberately interrupt another app’s microphone.
...
- `src/lib/audio-session.ts`: separate transient speech mode from ambient idle mode, remove the unsafe `auto` fallback from speech restoration, and observe supported audio-session state changes.
...
This delivers the strongest standards-based behavior available to an iPhone website. Unlike a native iPhone app, Orby cannot set Apple’s private `duckOthers` option or guarantee sharing with Voice Memos. If iOS 27 ignores `transient` for `speechSynthesis`, the safe fallback is mixing with `ambient`; on browsers without this experimental control, Orby cannot override the operating system’s choice. Orby will never reintroduce exclusive playback or a looping silent-audio workaround.
