...
const PLANNER_MODEL = Deno.env.get("PLANNER_MODEL") ?? "gpt-6-luna";
...
const TOOL_HANDLERS: Record<string, any> = {
  ...
  async generate_text(args, _ctx) {
    const apiKey = Deno.env.get("OPENAI_API_KEY");
    if (!apiKey) throw new Error("Missing OPENAI_API_KEY");
    const model = Deno.env.get("PLANNER_MODEL") ?? "gpt-6-luna";
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({
...
async function composeWrapUp(
  ...
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        model: Deno.env.get("PLANNER_MODEL") ?? "gpt-6-luna",
        messages: [
